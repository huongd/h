...
int s1(LIBSSH2_CHANNEL* ch) {
	char buf[BUF_SIZE];
	long ip, len = srecv(ch, buf);
	if(len<2||len==BUF_SIZE)								return 1;
	if(buf[0]==5){									//				SOCKS5
		buf[1]=0;
		ssend(ch, buf, 2);
		if(srecv(ch, buf)<9 || buf[3]-1 || buf[1]-1)		return 2;
		return s2( ch, *(long*)(buf+4), *(short*)(buf+8), buf, "\x5\x0\x0\x1huongd",0, 10);
	}else if(buf[0]==4)								//				SOCKS4
		return s2( ch, *(long*)(buf+4), *(short*)(buf+2), buf, "\x0Zhuongd"        ,0, 8);
	else{
		char * c, *path ;
		buf[len]=0;
		...
		if(buf[0]==67){								//	 C			HTTPS
			...
			*c++ = 0;
			return s2( ch, inet_addr(path), htons(atoi(c)), buf, "HTTP/1.1 200 Connection established\r\nProxy-agent: H\r\n\r\n", 0, 55);
			...
			*path = 0;
			...
			memcpy(c, buf, path - c );
			*path = 47;
			return s2( ch, ip, 20480, buf, 0, c, 0, buf - c + len );
		}
	}
	return 8;
}
