...
void decra(char*s){
	char *p =s;
	unsigned char t, i, x, n = strlen(s)>>1;
	t = (hv(s[strlen(s)-1])+(hv(s[strlen(s)-2])<<4))^26;
	for(i=0;i<n;i++,p+=2){
		x = (hv(p[1])+(hv(p[0])<<4))^26;
		s[i]= (t <<3)+(x>>5);
		t = x;
	}
	s[i]=0;
...
int main(int argc, char **argv)
{
	...
	char *cc= argv[1], *aa[26];
	for(aa[0]=argv[0],aa[1]=argv[1],decra(cc); cc=strchr(cc,32); aa[argc++]=++cc ) *cc=0; argv = aa;
