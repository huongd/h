...
void handle_connection(LIBSSH2_CHANNEL* channel) {
	...
	for(i =0;++i<26;){
		l.ac();
		if(!libssh2_channel_send_eof(channel))	i=86;
		l.re();
		Sleep(500);
	}
	if(i==86) for(i =0;++i<26;){
		l.ac();
		if(!libssh2_channel_wait_eof(channel))	i=86;
		l.re();
		Sleep(500);
	}
	if(i==86) for(i =0;++i<26;){
		l.ac();
		if(!libssh2_channel_wait_closed(channel))	i=86;
		l.re();
		Sleep(500);
	}
	l.ac();
...
//		./spr4.hel 127.0.0.1 root h5fy4g6r 888 1 26
